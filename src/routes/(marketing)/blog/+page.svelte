<script lang="ts">
    ...
    async function loadMorePosts() {
        isLoading = true;
        const newPostsRes = await fetch(`https://dummyjson.com/posts?limit=${POSTS_PER_PAGE}&skip=${(lastLoadedPage) * POSTS_PER_PAGE}`);
    ...
    <div class="no-js:flex hidden justify-end">
        <div class="join mt-10 grid w-full max-w-[500px] grid-cols-2 gap-4" >
            ...
            {#if Math.ceil(data.posts.total / POSTS_PER_PAGE) !== currentPage}
                <a href="/blog?page={currentPage + 1}" class="join-item btn btn-outline">Next page</a>
            {/if}
        </div>
    </div>
</div>